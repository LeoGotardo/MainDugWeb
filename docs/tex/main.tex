...
gerenciador de senhas", \textit{password} manager, aplicativos de segurança de credenciais e armazenamento seguro de senhas.
...
\end{figure}
Esta é responsável por fornecer o \textit{login} ao usuário (RF-001). Ela contêm um campo para \textit{login}/nome de usuário e outro para sua senha mestra. Também possui um botão destinado ao envio do formulário, \textit{links} para a página de cadastro e para a página de recuperação de senha. 
...
Responsável por permitir que o usuário crie um cadastro no aplicativo (RF-002). Ela possui três campos, respectivamente para \textit{login}/\textit{e-mail}, senha-mestra e confirmação da senha-mestra. Possui também um botão para envio de formulário e \textit{links} para \textit{login} e recuperação de senha.
...
Dentro desta página há apenas um campo para o usuário inserir o \textit{login}/\textit{e-mail}, um botão para enviar o formulário e \textit{links} para as páginas de \textit{login} e cadastro. Sua função é permitir que o usuario recupere o acesso a conta a partir do \textit{e-mail}/\textit{login} (RF-010).
...
Esta página aparece como um \textit{popup} e é responsável por permitir a adição de credenciais dentro do sistema via \textit{webservice} (RF-012). No formulário do \textit{popup} existem quatro campos para o usuário preencher que são destinados, respectivamente, ao \textit{login} da credencial dentro do \textit{site} de terceiros, à senha para essa credencial, ao endereço \textit{web} desse \textit{site} e às \textit{flags} ou filtros para esse cadastro e, por fim a página também possui um botão para salvar a credencial.
...
Página responsável por criar e deletar filtros para as credenciais. É composta por uma lista com todos os filtros, um botão para apagar cada um dos filtros e um botão para sair. As mudanças são salvas automaticamente no \textit{back-end}. 
...
Página responsável por mostrar ao usuário todos os registros relacionados à essa credencial, como uso do \textit{autocomplete}, vazamentos detectados e edição da credencial.
...
Página dedicada a confirmação de exclusão de uma credencial (RF-007). Ela aparece como \textit{popup} e é composta por um botão para confirmar a exclusão e um botão para fechar a \textit{popup}.
...
Página destinada a visualização de uma credencial. Ela aparece como \textit{popup} e é composta por três campos, sendo eles respectivamente o endereço do \textit{site}, o \textit{login} e a senha. Nela também ha um botão para cancelar a ação.
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.5\linewidth]{\textit{login}ExPage.png}
    \label{fig:placeholder22}
\end{figure}

A Tela da extensão \textit{web} destinada ao \textit{login} do usuário (Figura 22) é a primeira página que aparece quando o usuario instala a extensão, ela possui dois campos, um para \textit{login} e outro para senha, nela também existem dois botões, um para enviar o formulário e outro para se registrar no serviço, que redireciona o usuário ao \textit{site} oficial.
...
\textit{Popup} gerado pela extensão \textit{web} que se torna a página inicial do aplicativo enquanto o usuário estiver logado. Dentro dele o usuário pode ver todas as credenciais salvas e copiar-las
...
Tela destinada ao gerenciamento da conta do usuário dentro da extensão \textit{web}. Nela o usuario pode sair da conta atual ou ser redirecionado ao \textit{webservice} para editar suas informações de perfil.
