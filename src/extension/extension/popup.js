    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            if (tabName === 'passwords' && currentUser) { // Corrigido para carregar apenas se logado
                loadPasswords(); // Apenas carrega se logado
            }
        });
    });
}
...
function setupEventListeners() {
    ...
    document.getElementById('deleteAccountBtn').addEventListener('click', () => {
        document.getElementById('deleteAccountStatus').innerHTML = '<div class="form-group"><label for="confirmPassword">Confirme sua senha para excluir:</label><input type="password" id="confirmPassword" required></div><button id="confirmDelete" class="btn danger">Confirmar Exclusão</button>';
        document.getElementById('deleteAccountStatus').style.display = 'block';
...
async function handleChangePassword(e) {
    ...
    if (!currentPassword || !newPassword) {
        showStatus('changePasswordStatus', 'Preencha todos os campos', 'error');
        return;
    }

    showStatus('changePasswordStatus', 'Alterando senha...', 'info');
    ...
    try {
        ...
        if (response.ok) {
            showStatus('changePasswordStatus', 'Senha alterada com sucesso!', 'success');
            document.getElementById('changePasswordForm').reset();
        } else {
            showStatus('changePasswordStatus', data.error || 'Erro ao alterar senha', 'error');
        }
    } catch (error) {
        showStatus('changePasswordStatus', 'Erro de conexão', 'error');
    }
}

// Handler de exclusão de conta
async function handleDeleteAccount() {
    const password = document.getElementById('confirmPassword').value;
    if (!password) {
        showStatus('deleteAccountStatus', 'Você precisa confirmar sua senha.', 'error');
        return;
    }

    showStatus('deleteAccountStatus', 'Excluindo conta...', 'info');
    ...
    try {
        ...
        if (response.ok) {
            ...
            handleLogout(); // Faz o logout após a exclusão
        } else {
            const data = await response.json();
            showStatus('deleteAccountStatus', data.error || 'Erro ao excluir conta', 'error');
        }
    } catch (error) {
        showStatus('deleteAccountStatus', 'Erro de conexão', 'error');
    }
}
...
function showAuthenticatedUI() {
    ...
    if (currentUser) {
        ...
        // Mostrar abas de usuário logado e esconder a de login
...
function showUnauthenticatedUI() {
    ...
    // Esconder abas de usuário logado e conta
